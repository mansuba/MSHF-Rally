...
if (!function_exists('store_cp_url')) {
    /**
     * Generate a Store control panel URL.
     *
     * HTML entities are escaped automatically.
     */
    function store_cp_url($controller = null, $method = null, array $params = array())
    {
        // $params can be passed as second parameter
        if (is_array($method)) {
            $params = $method;
            $method = null;
        }

        // skip legacy URL params
        foreach (array('D', 'C', 'M', 'S') as $key) {
            unset($params[$key]);
        }
        ...
        $params = array_merge(array_filter($defaults), $params);
        $query_string = e(http_build_query($params, '', '&'));

        return cp_url('addons_modules/show_module_cp', $query_string);
    }
}
    ...
    function store_setting_input($key, $default, $value)
    {
        ...
        if (is_array($default)) {
            ...
            switch ($default['type']) {
                ...
                case 'timezone':
                    return ee()->localize->timezone_menu($value, $input_name);
                default:
...
if (!function_exists('store_html_elem')) {
    /**
     * Generate a safely-escaped html element
     *
     * @param string $name The element name (alphanumeric characters only)
     * @param array $attributes Any element attributes
     * @param string|null $content The element content
     * @param boolean $safe_content True if the content should be treated as HTML-safe
     *
     * @return string
     */
    function store_html_elem($tag_name, array $attributes = array(), $content = null, $safe_content = false)
    {
        // http://www.w3.org/TR/html-markup/syntax.html#syntax-elements
        if (preg_match('/[^A-Za-z0-9]/', $tag_name)) {
            throw new InvalidArgumentException(sprintf('Invalid tag name: %s', $tag_name));
        }

        $void = in_array($tag_name, array('area', 'base', 'br', 'col', 'command', 'embed', 'hr', 'img', 'input', 'keygen', 'link', 'meta', 'param', 'source', 'track', 'wbr'));

        $html = sprintf('<%s', $tag_name);
        foreach ($attributes as $key => $value) {
            if (is_bool($value)) {
                if ($value) {
                    $html .= sprintf(' %s', e($key));
                }
            } else {
...
if (!function_exists('store_camel')) {
    /**
