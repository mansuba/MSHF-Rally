...
class SearchTag extends AbstractTag
{
    public function parse()
    {
        ...
        if (!empty($this->params['entry_id'])) {
            $query->whereIn('store_products.entry_id', explode('|', $this->params['entry_id']));
        }

        // on sale
        if (!empty($this->params['search:on_sale'])) {
            $ee = $this->ee;
            $query->where(function($query) use (&$ee) {
                // products must be on sale, otherwise don't match
                $query->whereRaw('false');

                $member_group_id = (int) $ee->session->userdata['group_id'];
                foreach ($ee->store->products->get_active_sales() as $sale) {
