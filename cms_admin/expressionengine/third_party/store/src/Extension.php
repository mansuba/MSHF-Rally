...
class Extension
{
    ...
    {
        $this->ee->functions->redirect(store_cp_url('settings'));
    }
    ...
    public function cp_menu_array($menu)
    {
        ...
        $menu['store'] = array();
        $menu['store']['dashboard'] = store_cp_url();
        $menu['store'][] = '----';
        $menu['store']['orders'] = store_cp_url('orders');
        $menu['store']['customers'] = store_cp_url('customers');

        if ($this->ee->store->config->has_privilege('can_access_inventory')) {
            $menu['store']['inventory'] = store_cp_url('inventory');
        }

        $menu['store']['promotions'] = array();
        $menu['store']['promotions']['sales'] = store_cp_url('sales');
        $menu['store']['promotions']['discounts'] = store_cp_url('discounts');

        $menu['store']['reports'] = store_cp_url('reports');

        if ($this->ee->store->config->has_privilege('can_access_settings')) {
            $menu['store'][] = '----';
            $menu['store']['settings'] = store_cp_url('settings');
        }
